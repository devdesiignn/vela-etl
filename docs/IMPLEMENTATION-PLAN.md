...
- [x] Tested against hand-written stub extractors only (`tests/test_orchestrator.py`, 6 tests) — no real OCR/vision-LLM adapters yet. This isolates orchestration logic from adapter correctness.
...
- **Integration test — the pipeline must work stage-to-stage, not just within each stage.** Unit tests exist per stage today (`test_orchestrator.py`, `test_transform.py`, `test_load_client.py`), but nothing currently proves one stage's real output is actually consumable by the next. Audited during Phase 2 work and confirmed this gap is real: `test_orchestrator.py` never imports or calls `reconcile`/`validate`/`shape`/`emit`, so nothing proves `split_results()`'s `extraction_results` list is a valid `reconcile()` input, or that its `total_failure_reviews` list correctly reaches the final review set. This phase's integration test must run the full chain through and through, end to end:
