...
- **Extract** (`src/etl/extract/`) — one or more extractor plug-ins turn a receipt photo into an `ExtractionResult`: candidate field values plus a per-field confidence score. Extractors share one interface. Orchestration never branches on which one ran. Implemented: the extractor interface and orchestrator (Phase 2). Also implemented: two adapters proving both ends of the adapter-effort spectrum (Phase 5). One is Azure Document Intelligence (`prebuilt-receipt`, cloud, already receipt-shaped). The other is a RapidOCR-based adapter (raw OCR text, mapped by hand), hardened against 28 real receipt photos. Remaining adapters (EasyOCR, PaddleOCR, docTR, vision-LLMs) are Phase 7.
